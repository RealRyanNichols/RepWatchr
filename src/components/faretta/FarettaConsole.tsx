...
export default function FarettaConsole({ initialQuery = "" }: FarettaConsoleProps) {
  ...
  async function send(nextInput = input, kind: "chat" | "prompt_button" = "chat") {
    ...
    setIsAsking(true);
    setLiveError(null);
    collectFarettaInteraction({ kind, content: trimmed, metadata: { source: "faretta_console" } });
    setInput("");
    ...
    try {
      const response = await fetch("/api/faretta/chat", {
        ...
        body: JSON.stringify({
          ...
          history: messages,
          tier: "free",
          userContext: { page: "faretta_console" },
        }),
      });
      ...
      setMessages((current) => [...current, { role: "faretta", content: buildAnswer(trimmed) }]);
    } finally {
      setIsAsking(false);
    }
  }
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-[#2b3247] bg-[#0A0E1A] text-[#F4EFE4] shadow-2xl">
      <div className="border-b border-white/10 bg-[linear-gradient(135deg,#0A0E1A_0%,#131826_48%,#1E2538_100%)] p-5">
        ...
        <div className="mt-5">
          <FarettaSearchBox compact placeholder="Ask Faretta AI what to find..." />
        </div>
      </div>
          ...
          <div className="space-y-3">
            {messages.map((message, index) => (
              <div
                ...
                className={message.role === "faretta" ? "rounded-2xl bg-white/10 p-4" : "ml-auto max-w-[86%] rounded-2xl bg-[#D4A855] p-4 text-[#0A0E1A]"}
              >
