  ...
  useEffect(() => {
    ...
    async function loadOfficeData() {
      const [
        ...
        countRows(supabase, "data_product_interests", { column: "status", value: "new" }),
        countRows(supabase, "political_feedback_responses"),
        countRows(supabase, "official_vote_reactions"),
      ...
      setLiveCounts([
        ...
        { label: "Data buyer leads", table: "data_product_interests", count: dataProductLeads.count, status: dataProductLeads.count === null ? "red" : dataProductLeads.count > 0 ? "yellow" : "green", detail: dataProductLeads.error || "New political data, report, panel, profile, and partnership inquiries." },
        { label: "Political feedback", table: "political_feedback_responses", count: feedbackResponses.count, status: feedbackResponses.count === null ? "red" : feedbackResponses.count > 0 ? "green" : "yellow", detail: feedbackResponses.error || "Verified account responses to official, issue, race, and score questions." },
        { label: "Vote reactions", table: "official_vote_reactions", count: voteReactions.count, status: voteReactions.count === null ? "red" : voteReactions.count > 0 ? "green" : "yellow", detail: voteReactions.error || "Bill and vote-level approval, disapproval, and context requests." },
