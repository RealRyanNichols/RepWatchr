...
export function SalesRepSignalWaitlist({ source = "repwatchr" }: { source?: string }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
      <form onSubmit={submit} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-red-700">Opt-in signal profile</p>
        <h2 className="mt-2 text-2xl font-black text-blue-950">Build a Sales Rep Signal preview.</h2>
        ...
        <div className="mt-5 grid gap-3">
          ...
          {status ? <p className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm font-bold text-blue-950">{status}</p> : null}
        </div>
      </form>
      ...
      <section className="rounded-lg border border-slate-200 bg-blue-950 p-5 text-white shadow-sm">
        ...
        <div className="mt-5 rounded-lg border border-white/10 bg-white/10 p-5">
          <div className="flex items-end justify-between gap-4">
            ...
            <span className="pb-2 text-sm font-black uppercase tracking-wide text-blue-100">pilot only</span>
          </div>
...
function SignalBlock({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.06] p-4">
      ...
      <ul className="mt-2 space-y-1 text-sm font-semibold leading-6 text-slate-200">
