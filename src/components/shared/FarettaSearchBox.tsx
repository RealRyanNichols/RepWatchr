...
export default function FarettaSearchBox({
  ...
  function submitSearch(nextQuery = query) {
    ...
    if (!trimmed) return;
    collectFarettaInteraction({ kind: "search", content: trimmed, metadata: { source: "faretta_search_box" } });
    router.push(`/faretta-ai?q=${encodeURIComponent(trimmed)}`);
  }
    ...
    <div className="w-full">
      ...
      >
        <div className="flex gap-2">
          <label htmlFor="faretta-search" className="sr-only">
