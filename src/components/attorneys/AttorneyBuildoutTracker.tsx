...
function publicDefenseStatusClass(status: "source_mapped" | "profile_seeded" | "roster_needed" | "license_match_needed") {
  if (status === "profile_seeded") return "bg-emerald-100 text-emerald-800";
  if (status === "source_mapped") return "bg-blue-100 text-blue-800";
          ...
          <div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <Metric
                ...
                value={`${dashboard.publicDefenderProfiles}/${dashboard.publicDefenderSourceTarget}`}
                detail={`${dashboard.publicDefenderSourcesMapped} official defender office or directory sources mapped.`}
              />
          ...
          <div className="rw-card rw-card-red rounded-2xl p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h3 className="text-xl font-black text-slate-950">Offices first. People after bar match.</h3>
              </div>
              <p className="text-xs font-bold text-slate-500">{dashboard.publicDefenderSeededStates} states seeded</p>
            </div>
            <div className="mt-3 grid max-h-[520px] gap-2 overflow-y-auto pr-1">
              {dashboard.publicDefenderSources.map((source) => (
                <a
                  key={source.id}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] font-black uppercase tracking-wide text-slate-500">
                        {source.state} · {source.scope}
                      </p>
                    </div>
                    <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-wide ${publicDefenseStatusClass(source.status)}`}>
                      {source.status.replace(/_/g, " ")}
                    </span>
                  </div>
                  <p className="mt-2 line-clamp-2 text-xs font-semibold leading-5 text-slate-600">{source.notes}</p>
                </a>
              ))}
            </div>
          </div>
