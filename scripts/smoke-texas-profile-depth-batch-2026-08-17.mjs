...
for (const relative of files) {
  ...
  if (isFederal && !profile.sourceLinks?.some((item) => item.url.includes("commons.wikimedia.org") && item.supports?.includes("portrait_license"))) errors.push(`${label}: Commons license ledger missing`);
...
console.log(`Texas August 17 profile-depth smoke passed for ${files.length} profiles.`);
