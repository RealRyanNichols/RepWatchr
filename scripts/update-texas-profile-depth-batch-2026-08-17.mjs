...
const housePortraits = new Map([
  [20, { profileId: "56161", url: "https://www.txdirectory.com/files/photo/per56161.jpg" }],
  [39, { profileId: "24923", url: "https://www.txdirectory.com/files/photo/per24923.jpg" }],
  [103, {
    url: "https://directory.texastribune.org/static/images/headshots/Rafael-Anchia.jpg",
    sourcePage: "https://www.texastribune.org/directory/rafael-anchia/",
    ...
    rights: "Stored with Texas Tribune attribution and directory provenance; reuse rights should be rechecked before redistribution outside RepWatchr.",
  }],
  [119, { profileId: "66162", url: "https://www.txdirectory.com/files/photo/per66162.png" }],
  [125, { profileId: "63033", url: "https://www.txdirectory.com/files/photo/per63033.jpg" }],
  [126, { profileId: "62365", url: "https://www.txdirectory.com/files/photo/per62365.png" }],
  [133, { profileId: "69369", url: "https://www.txdirectory.com/files/photo/per69369.png" }],
  [136, { profileId: "62491", url: "https://www.txdirectory.com/files/photo/per62491.jpg" }],
  [146, { profileId: "71989", url: "https://www.txdirectory.com/files/photo/per71989.jpg" }],
  [147, { profileId: "68455", url: "https://www.txdirectory.com/files/photo/per68455.png" }],
]);
...
const federalPortraitOverrides = new Map([
  ["M001157", "https://commons.wikimedia.org/wiki/Special:FilePath/Michael%20McCaul%20Official.jpg"],
]);
...
async function buildHouseProfiles() {
  ...
  for (const [district, portraitMeta] of selectedHousePortraits) {
    ...
    for (const entry of [
      ...
      source(portraitMeta.sourcePage ? "Texas Tribune portrait-publisher terms" : "Texas State Directory roster and publisher context", portraitMeta.sourcePage ? TRIBUNE_TERMS : TX_DIRECTORY_ROOT, ["portrait_provenance", "rights_recheck"]),
...
async function buildFederalProfiles() {
  ...
  for (const { district, file, profile } of profiles) {
    ...
    const latestVoteDate = record.votes.reduce((latest, vote) => vote.date > latest ? vote.date : latest, "");
    profile.bio = `${profile.name} represents Texas's ${district}${district === 2 ? "nd" : district === 3 ? "rd" : "th"} Congressional District in the U.S. House. The current Clerk profile verifies the office, contact details, party, and assignments to ${clerk.committees.join(" and ")}. RepWatchr links the official sponsorship and roll-call records and does not turn source availability into an uncited performance judgment.`;
