...
RepWatchr uses LeadRep Orchestrator v0 to pass verification and revenue work between Codex, GitHub, Supabase, Grok/xAI, and Vercel without auto-publishing or spending API credits by default.

## Connected Surfaces

- GitHub: `RealRyanNichols/RepWatchr`
- Vercel: `realryannichols/repwatchr`
- Supabase: `https://rgxboswrinsuakxqstyc.supabase.co`
- Branch: `main`
...
5. If approval is cleared and `LEADREP_GROK_MODE=api`, Grok/xAI receives the prompt and returns structured verification and package output.
6. Results are written to Supabase `agent_results`, with package concepts in `package_ideas` and verification signals in `predictive_signals` when present.
...
- Supabase: private agent bus, memory, run history, approval queue, predictive signals, proof results, and package ideas.
- Grok/xAI: optional reasoning layer for verification gaps, source patterns, package ideas, objections, and predictive signal review.
...
LEADREP_BUS_URL=https://rgxboswrinsuakxqstyc.supabase.co
...
```bash
LEADREP_GITHUB_REPOSITORY=RealRyanNichols/RepWatchr
LEADREP_VERCEL_PROJECT=realryannichols/repwatchr
...
- publishing a page, finding, profile update, or package
- emailing, texting, posting, or contacting anyone
- changing pricing or checkout behavior
- pushing public claims
- moving a verification idea into a sellable offer
...
- record current verified package inventory, traffic, conversion points, and recurring revenue
...
- predictive or verification signals accepted
...
- Grok-assisted verification packages create a repeatable conversion path Ryan wants to keep testing.
